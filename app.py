...
@app.post("/v1/recommend", response_model=RecommendResponse, dependencies=[Depends(require_key)])
...
@app.post("/v1/recommend_addr", response_model=RecommendResponse, dependencies=[Depends(require_key)])
async def recommend_addr(req: RecommendAddrRequest):
    ...
    return await recommend(rr)

@app.get("/v1/quick", dependencies=[Depends(require_key)])
async def quick(origin_lat: float, origin_lon: float, dest_lat: float, dest_lon: float, preferred_dest_station_name: str = "W 58 St & 11 Ave"):
    rr = RecommendRequest(
        origin={"lat": origin_lat, "lon": origin_lon},
        destination={"lat": dest_lat, "lon": dest_lon},
    )
    setattr(rr.prefs, "preferred_dest_station_name", preferred_dest_station_name)
    res = await recommend(rr)
